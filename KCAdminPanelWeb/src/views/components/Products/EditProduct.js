...
function EditProduct(props) {
    ...
    const getProductById = async () => {
        ...
        if (response.status) {
            formik.setValues({
                ...
                categoryId: response.data.categoryId.id,
                ...
                slabs: response.data.slabs,
            })

            if(response.data.productImage) {
                const url= API_BASE_URL+ '/' + response.data.productImage
                setImagePath(url)
    ...
    const addSlab = (i) => {
        formik.values.slabs.push({ from: 0, to: 0, unitPrice: 0 })
        forceUpdate();
    }
