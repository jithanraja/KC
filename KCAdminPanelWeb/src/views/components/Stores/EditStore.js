...
function EditStore(props) {
    ...
    const getCurrentStore = async() => {
        const response = await makeAPICall({
            endpoint: 'getStoreById',
            pathParam: `/${props.storeId}`
        })
        if(response.status) {
            formik.setValues({
                ...
                altContactNo       : response.data.altContactNo,
                address: {
                    ...
                    location : response.data.address.location.coordinates?.join(','),
                }
            })
            setLocationValue(response.data.address.location.coordinates?.join(','))
            setDistricts(cities.filter(item => item.state_name === response.data.address.state))
        } else {
            toast.error('Something went wrong, Unable to fetch the store data!')
        }
    }

    const updateStore = async(postData) => {
                ...
                <Row className="my-4">
                    ...
                    <Col md={6} sm={12}>
                        ...
                        <select className="form-control"
                            ...
                            {states?.map((item, idx) => {
                                return (
                                    formik.values.address.state === item.name ?
                                        <option key={`${idx}_states`} value={idx} selected>{item.name}</option>
                                        : <option key={`${idx}_states`} value={idx}>{item.name}</option>
                                )
                            })}
                        </select>
                    ...
                    <Col md={6} sm={12}>
                        ...
                        <select className="form-control"
                            ...
                            {districts?.map((item, inx) => {
                                return <option key={`${inx}_districts`} value={item.name} selected={formik.values.address.district.toLowerCase() === item.name.toLowerCase()}>{item.name}</option>
                            })}
                        </select>
